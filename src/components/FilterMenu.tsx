...
type Priority = 'all' | 'high' | 'med' | 'low'

interface FilterMenuProps {
  currentFilter: Priority
  onChange: (filter: Priority) => void
}

export function FilterMenu({ currentFilter, onChange }: FilterMenuProps) {
  ...
  const getFilterLabel = (filter: Priority) => {
    switch (filter) {
  ...
  const handleSelect = (filter: Priority) => {
    onChange(filter)
    setIsOpen(false)
  }
  ...
  return (
    <div ref={menuRef} style={{ position: 'relative', zIndex: 1000 }}>
      ...
      {isOpen && (
        ...
        >
          {(['all', 'high', 'med', 'low'] as Priority[]).map((filter) => {
